...
**The goal** for this project is to reproduce a econometric analysis on The effect of financial ratios towards Earnings per Share. Previously it was done in Stata. for this time, we will perform the analysis through R.

The econometric model is a panel data model which requires hypothesis testing and model selection. The target would be to verify the robustness of the estimation result through following steps :

**First, to verify the significance of the estimators to see if there are gonna be any inconsistencies.**

**Second, to add a new dependent variable:"logarithm of total asset" to represent company's size and check for consistency.**

**Third, to apply two more models: "Heteroskedasticity-Consistent Covariance Matrix Estimation" and "Panel-Corrected Standard Error".**

Based on the result, this research is designed to verify following hypothesis:
...
phtest(fixed, random)
```
Since the P-value from Hausman test is 0.3056. For a = 0.05, we can not reject the null hypothesis. Therefore random effect model is preferred than fixed effect model. 
...
```
Since the P-value from Breusch-Pagan test is 0.00000000000000022. For a = 0.05, we should reject the null hypothesis. Therefore individual effect is significant and random effect model is preferred than OLS model. We should evaluate random effect model compared to the other two models.
...
pbgtest(random)
```
Since the P-value from Breusch-Godfrey/Wooldridge test is 0.00000000000000022. For a = 0.05, we should reject the null hypothesis. Therefore there is a serial correlation problem from the estimated random model.

```{r}
# Testing for heteroskedasticity
bptest(EPS~ROA+Dbequrt+LOGTA, data=dataset, studentize=TRUE)
```
Since the P-value from Breusch-Pagan test is 0.00000000000000022. For a = 0.05, we should reject the null hypothesis. Therefore there is a heteroskedasticity problem from the estimated random model besides the serial correlation problem. According to *MacKinnon J. G., White H.* (1985),**Heteroskedasticity-Consistent Covariance Matrix Estimation **method might solve there problems..
...
Table above presents the regression result step by step with pooled ordinary least squared (POLS), fixed effect (FE), random effect (RE)， Random effect model with robust Standard Error. Since the existing of Heteroskedasticity and autocorrelation issues, Random effect model with robust Standard Error method should be made for this model.This study will focus on its result and compare the differences among other models. 
...
**logarithm of total asset** has p-value of 0.009 (0.009< 0.01). So, at the significance level of 0.01, logarithm of total asset (company size) has positive influence on earnings per share. If LOGTA increases by one precent and other variables stay the same, EPS will increase by 0.092. 
